...
public class ReferencedRuleInEPIsAnInterface implements LanguageComponentBaseASTLanguageComponentCoCo {
    @Override
    public void check(ASTLanguageComponent node) {
        ...
        for (final ASTRequiredExtension extensionPoint : node.getExtensionPoints()) {
            ...
            if (node.getGrammarExtensionPoint(extensionPoint.getName()).isPresent()) {
                if (!prodWithInherited.getClass().isInterface()) {
                    Log.error(String.format(
                            "MC004 The referenced rule %s of the abstract syntax extension point %s " +
                                    "in language component %s is not an interface rule.",
                            ruleName,
