...
/**
 * Adapter class between producer interface of the extension point ${source.name} and
 * the producer interface of provision point ${target.name}. This adapter has to be extended by a
...
public abstract class ${source.name}2${target.name}TOP implements ${target.name} {
    ...
    public Class<?> getTargetInterface() {
        return ${productAdapterName}.class;
    }
}